...
with tab_entry:
    ...
    # 8. Sponsored Projects (NOW WITH YES/NO)
    ...
    if has_sponsored == "Yes":
        st.caption("Upload sanction letter is required per sponsored project. Completion is required only if completed.")
        ...
        proj_confirm = True  # no need to confirm if section not used

    st.divider()

    # 9. Consultancy Work (NOW WITH YES/NO)
    ...
    if has_consultancy == "Yes":
        st.caption("Same fields as Sponsored Projects. PDFs are optional here.")
    ...
    if submitted:
        ...
        # Publications (required + PDF required)
        ...
        # Patents/models/prototypes required only if selected Yes
        if has_patents == "Yes":
        ...
        # Sponsored projects (if Yes): sanction PDF required
        if has_sponsored == "Yes":
        ...
        # Consultancy (if Yes): required fields, PDFs optional
        if has_consultancy == "Yes":
        ...
        if errors:
            ...
            # Patents/Models/Prototypes (PDF optional)
            if has_patents == "Yes":
                for pm in st.session_state["patents_models"]:
                    rid = pm["_id"]
            ...
            # Sponsored Projects (if Yes) + PDFs
            if has_sponsored == "Yes":
                for sp in st.session_state["sponsored"]:
                    rid = sp["_id"]
                    ...
                    """, (
                        faculty_id,
                        sp["project_date"].isoformat() if sp.get("project_date") else "",
            ...
            # Consultancy Work (if Yes) + PDFs optional
            if has_consultancy == "Yes":
                for cw in st.session_state["consultancy"]:
                    rid = cw["_id"]
                    ...
                    """, (
                        faculty_id,
                        cw["project_date"].isoformat() if cw.get("project_date") else "",
